...
def clockinitilization():
    ...
    boardwithmodules.write('BOARD.0', 'AREA_SPI_DIV', int(0x20), int(0x58))
    time.sleep(0.01)

    # Save send data
    boardwithmodules.write('BOARD.0', 'AREA_SPI_DIV', int(0x81), int(0x5A))
    time.sleep(0.01)
...
def configureadcs():
    ...
    boardwithmodules.write('BOARD.0', 'WORD_ADC_ENA', 0)
    time.sleep(1)  # Wait for 1 second
...
def configuretiming(timing_frequency):
    ...
    boardwithmodules.write('APP.0', 'WORD_TIMING_FREQ', 0, 7)
    ...
    boardwithmodules.write('APP.0', 'WORD_TIMING_INT_ENA', int(0x81))
    ...
    boardwithmodules.write('APP.0', 'WORD_DAQ_MUX', 0, 0)
...
def external_clock_initilization(PLLsetting):
    ...
    initial_clockFrequency = boardwithmodules.read("BOARD.0", "WORD_CLK_FREQ", 1, 1)
    ...
    boardwithmodules.write('BOARD.0', 'WORD_CLK_MUX', 0, 0)  # Mux 1A (Choose RTM_CLK2)
    time.sleep(0.01)  # Wait for 10ms
    boardwithmodules.write('BOARD.0', 'WORD_CLK_MUX', 0, 1)  # Mux 1B (Choose RTM_CLK2)
    time.sleep(0.01)
    boardwithmodules.write('BOARD.0', 'WORD_CLK_MUX', 0, 2)  # Mux 2A
    time.sleep(0.01)
    boardwithmodules.write('BOARD.0', 'WORD_CLK_MUX', 0, 3)  # Mux 2B
    ...
    clockFrequency = boardwithmodules.read("BOARD.0", "WORD_CLK_FREQ", 1, 1)
